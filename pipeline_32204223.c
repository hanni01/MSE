...
void fetch() {
	...
	IF_ID_latch[0].inst = memory[pc / 4];
...
void memory_access() {
	...
	if (EX_M_latch[1].cs.MemRead == 1) {
		printf("0x%08x \n", EX_M_latch[1].ALUResult);
		M_WB_latch[0].LMD = memory[EX_M_latch[1].ALUResult / 4];
	...
	if (EX_M_latch[1].cs.MemWrite == 1) {
		memory[EX_M_latch[1].ALUResult / 4] = EX_M_latch[1].rt_v;
...
int main(int argc, char *argv[]) {
	...
	//handling input args
	char *input_file = "C:\\Users\\he308\\OneDrive\\바탕 화면\\JAVA\\workspace\\Mips_single_cycle\\simple3.bin";
	FILE *fp;
	int ret;
	int fin;
	int index = 0;
